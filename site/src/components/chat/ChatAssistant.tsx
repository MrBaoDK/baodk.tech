      ...
      <div className="w-full max-w-5xl h-full flex flex-col bg-white/[0.01] backdrop-blur-3xl border border-white/10 md:rounded-[0.5rem] shadow-2xl shadow-black/80 overflow-hidden relative">
        ...
        <div className="h-4 bg-white/[0.03] border-b border-white/5 flex items-center justify-between px-6 md:px-10 pointer-events-none select-none shrink-0">
          <div className="flex gap-4 md:gap-8">
            <span className="text-[5px] md:text-[7px] font-black text-white/10 uppercase tracking-[0.3em]">Session://BAO-NTEL-V4.0</span>
            <span className="text-[5px] md:text-[7px] font-black text-white/10 uppercase tracking-[0.3em] hidden sm:inline">Protocol: Neural-TLS 1.3</span>
          </div>
          <div className="flex gap-4 md:gap-8">
            <span className="text-[5px] md:text-[7px] font-black text-[var(--color-primary)]/30 uppercase tracking-[0.3em]">Secure_Channel: Active</span>
            <span className="text-[5px] md:text-[7px] font-black text-white/10 uppercase tracking-[0.3em]">Latency: 14ms</span>
          </div>
        </div>
        ...
        <div className="p-2 md:p-4 border-b border-white/5 flex justify-between items-center bg-white/[0.02] relative z-20 shrink-0">
          <div className="flex items-center gap-3 md:gap-4">
            ...
            >
              <span className="material-symbols-outlined text-xs md:text-sm text-white/40 group-hover:text-white transition-colors">arrow_back</span>
            </button>
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-[5px] md:text-[7px] text-white/40 uppercase tracking-[0.3em] font-black">Neural Core Active</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-8 opacity-40">
              <div className="w-14 h-14 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center mb-4 rotate-6">
                <span className="material-symbols-outlined text-2xl text-[var(--color-primary)]">terminal</span>
              </div>
              <h4 className="text-base font-black text-white mb-2 tracking-tighter uppercase italic">System Initialized</h4>
            ...
            messages.map((msg, i) => (
              <div key={i} className={`flex gap-3 md:gap-4 ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-6 duration-700`}>
                <div className={`max-w-[90%] md:max-w-[95%] rounded-xl md:rounded-2xl p-3 md:p-5 text-sm md:text-base leading-relaxed relative ${msg.role === 'user'
